...
const TYPOGRAPHY = {
  ...
  size: { xs: 11, sm: 12, md: 13, lg: 14, xl: 16, h2: 20, h1: 24 },
...
const COLORS = {
  ...
  icon: {
    rest: '#717A8C', hover: '#4A5568', press: '#0F0F1A',
    restMuted: '#7B90AA', active: '#2563EB', activeHover: '#1D4ED8',
    activeBg: 'rgba(37,99,235,0.10)', mutedBg: 'rgba(123,144,170,0.10)',
    size: 16, dim: 38,
  },
};
const INPUT = {
  ...
  hover: { border: '1px solid #B0BCC8', shadow: '0 1px 4px rgba(0,0,0,0.08)' },
  focus: { border: '1px solid #4B7AC7', shadow: '0 0 0 3px rgba(37,99,235,0.10), 0 1px 4px rgba(0,0,0,0.05)' },
  select: {
    bg: '#FFFFFF', border: '1px solid #D1D5DB', borderRadius: 10, padding: '9px 14px',
    height: 38, fontSize: 14, color: '#111827', minWidth: 140,
    hover: { border: '1px solid #B0BCC8', shadow: '0 1px 4px rgba(0,0,0,0.08)' },
    focus: { border: '1px solid #4B7AC7', shadow: '0 0 0 3px rgba(37,99,235,0.10), 0 1px 4px rgba(0,0,0,0.05)' },
  },
};
const TABLE = {
  ...
  row: { borderBottom: '1px solid #F3F4F6', hoverBg: '#F8F9FB', pressedBg: '#DBEAFE', fontSize: 13, color: '#374151' },
  density: { compact: { headerPadding: '9px 16px', rowPadding: '9px 16px' }, relaxed: { headerPadding: '13px 20px', rowPadding: '14px 20px' } },
  cell: {
    plain:   { fontSize: 13, color: '#374151' },
    mono:    { fontFamily: "'JetBrains Mono', monospace", fontSize: 12, fontWeight: 400, color: '#4A5568' },
    monoId:  { fontFamily: "'JetBrains Mono', monospace", fontSize: 13, fontWeight: 600, color: '#2D3142' },
    numeric: { fontFamily: "'JetBrains Mono', monospace", fontSize: 14, fontWeight: 700 },
...
const SEARCHBAR = {
  light: {
    borderRadius: 25, inputFontSize: 15, placeholderFontSize: 13,
    textColor: '#1A1F35', accentColor: '#4B7AC7',
    resting: { bg: '#FFFFFF', border: '1.5px solid #E2E5EB', shadow: '0 1px 4px rgba(0,0,0,0.04)', iconColor: '#B0B8C8', placeholder: '#B0B8C8' },
    hover:   { bg: '#FCFCFD', border: '1.5px solid #BFCAD5', shadow: '0 2px 14px rgba(0,0,0,0.07)', iconColor: '#9BAABB' },
    focus:   { border: '1.5px solid #4B7AC7', shadow: '0 0 0 4px rgba(37,99,235,0.10), 0 4px 20px rgba(0,0,0,0.06)', iconColor: '#4B7AC7' },
    shortcutHint: { color: '#9CA3AF', bg: '#F5F7FA', border: '1px solid #E2E5EB' },
    pasteBtn: {
      bg: '#F5F7FA', border: '#E2E5EB', color: '#9CA3AF',
      hover: { bg: 'rgba(123,144,170,0.09)', border: 'rgba(123,144,170,0.35)', color: '#5A6B7E' },
    },
  },
};
const TOAST = {
  ...
  variants: { success: { bg: '#22C55E', color: '#FFFFFF' }, error: { bg: '#EF4444', color: '#FFFFFF' }, info: { bg: '#2D3142', color: '#FFFFFF' } },
};
const DASH_WIDGET = {
  title: { fontSize: 11, fontWeight: 700, textTransform: 'uppercase' as const, letterSpacing: '0.07em', color: '#9CA3AF', margin: 0 },
  action: { display: 'inline-flex' as const, alignItems: 'center' as const, fontSize: 11, fontWeight: 600, color: '#9CA3AF', background: 'transparent', border: '1px solid #E5E7EB', borderRadius: 20, padding: '3px 10px', cursor: 'pointer' as const, fontFamily: 'inherit', lineHeight: 1, outline: 'none', flexShrink: 0 },
  actionHover: { background: 'rgba(123,144,170,0.10)', color: '#6B7280' },
};
const STAT_CARD = {
  label:  { fontSize: 11, fontWeight: 700, textTransform: 'uppercase' as const, letterSpacing: '0.07em', color: '#717A8C', marginBottom: 6 },
  number: { fontSize: 30, fontWeight: 700, lineHeight: 1, color: '#1A1F35', fontFamily: "'Sora', sans-serif" },
  bar: { height: 6, borderRadius: 999, width: 4, widthActive: 12 },
  accentColors: { grey: '#9CA3AF', orange: '#F59E0B', red: '#EF4444', green: '#10B981' },
};
const EMPTY_STATE = {
  wrapper: { padding: '48px 24px', display: 'flex', flexDirection: 'column' as const, alignItems: 'center' as const, justifyContent: 'center' as const, gap: 8 },
  icon:     { size: 28, color: '#D1D5DB' },
  title:    { fontSize: 13, fontWeight: 600, color: '#9CA3AF', margin: 0 },
  subtitle: { fontSize: 12, color: '#B0B8C8', textAlign: 'center' as const, margin: 0 },
};
const SURFACE = {
  frame:   { bg: 'linear-gradient(160deg, #060C1A 0%, #0C1D3C 55%, #091630 100%)', padding: 8, borderRadius: 16 },
  ...
  content: { bg: '#FFFFFF', bodyBg: 'linear-gradient(180deg, #F5F6FA 0%, #EAEBF0 60%, #E3E4EA 100%)', padding: '24px 32px', gap: 20, panelTop: 8, borderRadius: 12, shadow: '0 4px 24px rgba(0,0,0,0.3), inset 0 2px 8px rgba(0,0,0,0.03), inset 0 -2px 8px rgba(0,0,0,0.02)' },
  header:  { bg: '#FFFFFF', borderBottom: '1px solid #ECEEF2', paddingX: 32, tabStrip: { marginTop: 24, tabPaddingX: 20, tabPaddingY: 9, fontSize: 13, default: { color: '#717A8C', fontWeight: 400, bg: 'transparent' }, hover: { color: '#4A5568', fontWeight: 400, bg: '#F5F7FA', borderRadius: '6px 6px 0 0' }, press: { color: '#2D3142', fontWeight: 600, bg: '#DBEAFE', borderRadius: '6px 6px 0 0' }, active: { color: '#1A1F35', fontWeight: 600, indicator: '#003C7E' } } },
  card:    { bg: '#FFFFFF', padding: 16, borderRadius: 12, border: '1px solid #E5E7EB', borderHover: '1px solid #D1D5DB', shadow: '0 1px 3px rgba(0,0,0,0.05)', shadowHover: '0 4px 12px rgba(0,0,0,0.10)', shadowPress: '0 2px 6px rgba(0,0,0,0.08)', gap: 24 },
  panel:   { bg: '#FFFFFF', padding: '12px 16px', borderRadius: 12, border: '1px solid #ECEEF2', shadow: '0 1px 2px rgba(0,0,0,0.02)' },
  sidebar: { bg: '#0A1528', borderRadius: 16, width: 64, widthExpanded: 240, paddingY: 12, paddingX: 8, iconColor: 'rgba(255,255,255,0.4)', iconColorActive: '#C6FF00', iconBgActive: 'rgba(198,255,0,0.08)', iconBgHover: 'rgba(255,255,255,0.06)', divider: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.07)', shadow: 'inset 0 1px 0 rgba(255,255,255,0.06)' },
  menuItem: { padding: '9px 12px', borderRadius: 8, fontSize: 13, gap: 8, marginBottom: 2, transition: 'all 0.12s', default: { bg: 'none', color: '#4A5568', fontWeight: 400 }, hover: { bg: '#F5F7FA', color: '#4A5568', fontWeight: 400, transform: 'translateX(2px)' }, press: { bg: '#DBEAFE', color: '#2D3142', fontWeight: 600 }, danger: { default: { color: '#EF4444' }, press: { bg: '#FEE2E2', color: '#EF4444' } } },
  tokenRow: { labelColor: '#5C6578', valueColor: '#111827', bg: '#FAFBFC', borderRadius: 10, padding: '10px 16px', border: '1px solid #D1D5DB' },
  sectionLabel: { fontSize: 11, fontWeight: 700, textTransform: 'uppercase' as const, letterSpacing: '0.08em', color: '#5C6578', marginBottom: 16 },
};
